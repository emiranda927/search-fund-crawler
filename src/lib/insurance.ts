import { InsuranceMatch } from '../types.js';
...
export function analyzeInsurance(text: string): {
  ...
  function getProximityScore(text: string, term1: string, term2: string): number {
    const words = text.split(/\s+/);
    const positions1: number[] = [];
    const positions2: number[] = [];
    
    words.forEach((word, index) => {
      if (word.includes(term1.toLowerCase())) positions1.push(index);
      if (word.includes(term2.toLowerCase())) positions2.push(index);
    });
  ...
  for (const term of INSURANCE_TERMS.statements) {
    ...
    const regex = new RegExp(
      `\\b${lowerTerm.split(' ').join('[\\s-]*')}\\b|` +
      `\\b${lowerTerm.replace('insurance', '(insurance|coverage)')}\\b`,
      'gi'
    );
  ...
  const providerMatches: InsuranceMatch[] = [];
